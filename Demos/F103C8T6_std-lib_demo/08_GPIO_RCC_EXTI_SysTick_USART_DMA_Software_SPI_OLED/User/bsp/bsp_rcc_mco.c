...
void RCC_MCO_Init(void)
{
  /* 如果要修改，需要重新设置系统时钟，这时候可以选择使用HSE还是HSI */
    // 使用 HSE 时，SYSCLK = 8M * RCC_PLLMul_x, x:[2,3,...16],最高是128M
    // 使用 HSI 时，SYSCLK = 4M * RCC_PLLMul_x, x:[2,3,...16],最高是64MH
    ...
    // MCO 引脚初始化
    // MCO_GPIO_Config();
    // MCO引脚输出 HSE,HSI,PLLCLK/2,SYSCLK 来检测系统时钟是否配置准确。
    // RCC_MCOConfig(RCC_MCO_HSE);
    // RCC_MCOConfig(RCC_MCO_HSI);
    // RCC_MCOConfig(RCC_MCO_PLLCLK_Div2);
    RCC_MCOConfig(RCC_MCO_SYSCLK);   // recommended
}