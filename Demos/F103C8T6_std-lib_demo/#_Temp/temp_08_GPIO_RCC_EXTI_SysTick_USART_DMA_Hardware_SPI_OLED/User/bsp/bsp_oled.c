...
uint8_t oled_delay_flag = 0;
uint8_t oled_delay_counter = 0;

void OLED_Process_Loop()
{
    // printf("OLED_Process_Loop: into.\n");
    if (oled_delay_flag == 1)
    {
        oled_delay_counter += 1;
        // counter add each 500ms.
        if (oled_delay_counter == 1)
        {
            OLED_ShowPicture(0,0,128,8,BMP3);
            // SysTick_Block_Delay_ms(500);
        }
        else if (oled_delay_counter == 2)
        {
